...
class SilktideCookieBanner {
  ...
  /* ─── Helper: aplikuje paletu z  config.background.colors  ─── */
  SilktideCookieBanner.prototype.applyColors = function(){
    if(!this.config || !this.config.background || !this.config.background.colors){ return; }
    var c    = this.config.background.colors;
    var root = document.getElementById('silktide-wrapper');
    if(!root){ return; }
    if(c.primary){ root.style.setProperty('--cmp-primary',  c.primary); }
    if(c.bg)     { root.style.setProperty('--cmp-bg',       c.bg); }
    if(c.text)   { root.style.setProperty('--cmp-text',     c.text); }
    if(c.border) { root.style.setProperty('--cmp-border',   c.border); }
    if(c.switch) { root.style.setProperty('--cmp-switch',   c.switch); }
  };
